...
const ClienteList = (props) => {
	const clientes = props.clientes.map((cliente) => (
		<tr key={cliente._links.self.href}>
			...
			<td scope="row">
				<Link to={'/clientes/'+ cliente._links.self.href.split("/").slice(-1) + '/editar'}>Editar</Link>
			</td>
		</tr>
	));
		// ZZZZZZZZZZZZZ
...
const PedidoList = (props) => {
	const pedidos = props.pedidos.map((pedido) => (
		<tr key={pedido._links.self.href}>
			...
			<td scope="row">
				<Link to={'/pedidos/'+ pedido._links.self.href.split("/").slice(-1) + '/editar'}>Editar</Link>
			</td>
		</tr>
	));
...
const LibroList = (props) => {
	const libros = props.libros.map((libro) => (
		<tr key={libro._links.self.href}>
			...
			<td scope="row">
				<Link to={'/libros/'+ libro._links.self.href.split("/").slice(-1) + '/editar'}>Editar</Link>
			</td>
		</tr>
	));
